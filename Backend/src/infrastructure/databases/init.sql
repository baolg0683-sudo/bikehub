...
-----------------------------------------------------------
CREATE SCHEMA IF NOT EXISTS auth;         -- Người dùng & Số dư ví
CREATE SCHEMA IF NOT EXISTS listings;     -- Tin đăng & Chi tiết xe
CREATE SCHEMA IF NOT EXISTS inspections;  -- Báo cáo kiểm định
CREATE SCHEMA IF NOT EXISTS wallet;       -- Lịch sử Nạp/Rút/Dòng tiền
CREATE SCHEMA IF NOT EXISTS orders;       -- Đơn hàng & Ký quỹ (Escrow)
CREATE SCHEMA IF NOT EXISTS interactions; -- Chat & Đánh giá
...
CREATE TABLE auth.users (
    ...
    password_hash TEXT NOT NULL,           -- Dùng hash bảo mật hơn
    ...
    reputation_score FLOAT DEFAULT 5.0,    -- Điểm uy tín (từ bản remote)
    certificate_id VARCHAR(50),            -- Chứng chỉ cho Inspector (từ bản remote)
...
-----------------------------------------------------------
-- 3. WALLET SERVICE (Quản lý dòng tiền thực)
-----------------------------------------------------------
CREATE TABLE wallet.transactions (
    transaction_id SERIAL PRIMARY KEY,
    user_id INT REFERENCES auth.users(user_id),
    amount DECIMAL(15, 2) NOT NULL,
    -- TOPUP: Nạp, WITHDRAW: Rút, HOLD: Giam cọc, RELEASE: Thanh toán, REFUND: Hoàn tiền
    type VARCHAR(20) NOT NULL, 
    status VARCHAR(20) DEFAULT 'PENDING',  -- PENDING, SUCCESS, REJECTED
    bank_info JSONB,                       -- Thông tin ngân hàng đối soát
    evidence_url TEXT,                     -- Ảnh bill chuyển khoản
    admin_note TEXT,
    processed_by INT REFERENCES auth.users(user_id),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE orders.orders (
    ...
    status VARCHAR(20) DEFAULT 'WAITING_FOR_DEPOSIT', 
...
CREATE TABLE inspections.reports (
    ...
    technical_details JSONB,               -- Lưu linh hoạt: {frame: "Good", brakes: "Wear", ...}
    overall_verdict TEXT,
    is_passed BOOLEAN DEFAULT FALSE,
    report_url TEXT,                       -- Link file PDF/Ảnh chi tiết
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
